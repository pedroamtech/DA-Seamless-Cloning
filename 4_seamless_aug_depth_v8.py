...
# =============================================================================
# CONFIGURACIÓN V8 (Seamless Cloning Puro - Perez03)
# =============================================================================
    ...
    for bg_path in tqdm(bg_images, desc=f'V7 YOLO-Seg {partition}', ncols=100):
        ...
        for _, row in candidates.iterrows():
            ...
            for _ in range(5):
                ...
                # =====================================================================
                # SOLUCIÓN: SEAMLESS CLONING PURO (Perez et al. 2003)
                # =====================================================================
                # 1. Expandimos el recorte usando BORDER_REPLICATE para crear "asfalto sintético"
                pad_w = max(10, int(nw * 0.40))
                pad_h = max(10, int(nh * 0.40))
                crop_padded = cv2.copyMakeBorder(crop, pad_h, pad_h, pad_w, pad_w, cv2.BORDER_REPLICATE)
                
                # 2. Expandimos la máscara de YOLO
                mask_padded = cv2.copyMakeBorder(mask_yolo, pad_h, pad_h, pad_w, pad_w, cv2.BORDER_CONSTANT, value=0)
                
                # 3. DILATACIÓN: Creamos la "Máscara Holgada" recomendada en el paper
                # La frontera del clonado caerá en el asfalto sintético, NO en la ropa de la persona.
                kernel_size = max(5, int(min(nw, nh) * 0.20))
                kernel = np.ones((kernel_size, kernel_size), np.uint8)
                mask_dilated = cv2.dilate(mask_padded, kernel, iterations=1)
                
                # Convertimos a 3 canales
                src_mask = cv2.merge([mask_dilated, mask_dilated, mask_dilated])
                
                try:
                    # 4. Aplicamos Poisson Image Editing
                    result_img = cv2.seamlessClone(crop_padded, result_img, src_mask, (cx, cy), cv2.NORMAL_CLONE)
        ...
        out_name = f"{os.path.splitext(bg_name)[0]}_v8_{datetime.now().strftime('%H%M%S%f')}"
        cv2.imwrite(str(out_img_dir / (out_name + '.jpg')), result_img)
        with open(str(out_lbl_dir / (out_name + '.txt')), 'w') as f: f.write('\n'.join(final_labels))

if __name__ == '__main__':
    print("="*60)
    print("  Seamless Augmentation V8: Perez03 Seamless Cloning Puro")
    print("="*60)
